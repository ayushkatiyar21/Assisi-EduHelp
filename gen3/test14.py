...
class VisionChatApp:
    ...
    def _bubble_with_image(self, text: str, kind: str, image_path: str):
        ...
        # Prepare preview image (larger than the 96x96 sidebar preview)
        try:
            img = Image.open(image_path)
            # Max display size inside bubble (scaled with UI_SCALE)
            max_w = int(420 * UI_SCALE)
            max_h = int(320 * UI_SCALE)
            img.thumbnail((max_w, max_h))
            ctk_img = self._to_ctk_image(img, img.size)
            img_label = ctk.CTkLabel(bubble, image=ctk_img, text="")
            img_label.pack(padx=10, pady=(10, 6))
        except Exception as e:
            # If image fails to load, still render text bubble and note error subtly
    ...
    def browse_image(self):
        ...
        if not file_path:
            ...
            self.image_path = None
            if self.preview_row.winfo_manager():
    ...
    def _finish_request_cleanup(self):
        # Reset image state
        self.image_path = None
        # Hide preview row when request finishes
        if self.preview_row.winfo_manager():
            self.preview_row.pack_forget()
        # Reset preview content for next time
        self.root.after_idle(lambda: self.preview_label.configure(image=self.placeholder_image, text="No image"))
        self.image_preview = None

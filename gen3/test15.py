...
API_KEY = "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"
API_MODEL = "google/gemma-3-27b-it:free"
BASE_URL = "https://openrouter.ai/api/v1"

# Global UI scaling (increase/decrease overall size of all widgets and fonts)
UI_SCALE = 1.35  # 1.0 = default size; try 1.2, 1.35, 1.5, etc.

# Best resampling available for sharp previews
...
class VisionChatApp:
    def __init__(self, root: ctk.CTk):
        ...
        # Apply global scaling to all widgets and fonts
        ...
        self._stream_buffer = ""
        self._cursor_visible = True          # blinking state
        self._cursor_glyph = "│"             # thin vertical line cursor
        self._stream_label = None            # label used during streaming
        ...
        # Wraplength scaled to match overall UI scale
        self.wraplength = int(720 * UI_SCALE)

        # Placeholder image for preview box (kept ready but hidden until image is uploaded)
    ...
    def _build_header(self):
        ...
        if logo_path:
            try:
                ...
                # Header logo (already larger); will also be scaled by UI_SCALE
                self.logo_ctk_img = ctk.CTkImage(light_image=logo_img.copy(), dark_image=logo_img.copy(), size=(56, 56))
                ctk.CTkLabel(left, image=self.logo_ctk_img, text="").pack(side="left", padx=(2, 10))
            except Exception:
                self.logo_ctk_img = None

        # Larger title text (also scaled by UI_SCALE)
        title = ctk.CTkLabel(
            left,
            text="Assisi EduHelp",
    ...
    def _build_input_card(self):
        ...
        # Preview row (HIDDEN by default; will be shown only when an image is uploaded)
        self.preview_row = ctk.CTkFrame(self.input_card, fg_color="transparent")
        # Do NOT pack here; we pack when an image is selected
    ...
    def _bubble_with_image(self, text: str, kind: str, image_path: str):
        # Build a bubble that shows an image above the text (used for user's message with image)
        bg, border, txt, align = self._bubble_colors(kind)
        ...
        # Prepare crisp preview image (match CTk logical size with high-DPI pixels)
        try:
            pil = Image.open(image_path)
            max_w_logical = int(420 * UI_SCALE)  # logical space available (scaled with UI)
    ...
    def _prepare_ctk_image(self, pil_img: Image.Image, max_logical_size: tuple[int, int]) -> ctk.CTkImage:
        """
        Create a crisp CTkImage by:
        - resizing with LANCZOS to target pixel size (logical * UI_SCALE) using aspect-fit
        - passing the logical size to CTkImage so it doesn't upscale and blur
        """
        max_w_logical, max_h_logical = max_logical_size
        # Target pixel size that CTk will render at (logical * UI scale)
        target_px_size = (max_w_logical, max_h_logical)  # already includes UI_SCALE above
        img = pil_img.copy()
        img = ImageOps.contain(img, target_px_size, method=RESAMPLE_BEST)

        # Compute logical size from pixels (reverse of UI scale), keep at least 1x1
    ...
    def _hide_preview_row(self, reset=False):
        # Hide the input preview row immediately (optionally reset labels)
    ...
    def begin_streaming(self):
        # Create an empty bot bubble and start live updates with a thin blinking cursor
    ...
    def _animate_stream_update(self):
        # Update the bubble text with the streamed buffer + a thin blinking cursor
    ...
    def _init_client(self):
        try:
            ...
            # intentionally no success message to the chat
        except Exception as e:
            self.client = None
    ...
    def browse_image(self):
        ...
        if not file_path:
            # Hide preview row if user cancels selection
            self.image_path = None
        ...
        # Show the preview row (only now)
        if not self.preview_row.winfo_manager():
            self.preview_row.pack(fill="x", padx=12, pady=(12, 6))

        try:
            pil = Image.open(file_path)
            # Make the small preview crisp: fit image into 96x96 logical (x UI_SCALE pixels) using LANCZOS
    ...
    def start_query_thread(self, event=None):
        ...
        if self.image_path:
            # Image preview above the entered prompt (in chat bubble)
            self._bubble_with_image(user_input if user_input else "[Image Query]", "user", self.image_path)
            # Immediately hide the input preview row after sending (per request)
    ...
    def _finish_request_cleanup(self):
        # Reset image state
        self.image_path = None
        # Ensure preview row stays hidden after request completes
        self._hide_preview_row(reset=True)
        self.image_preview = None
